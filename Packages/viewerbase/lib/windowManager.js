...
function updateWindows(data) {
    ...
    var contentId = $(".tabTitle.active a").data('target');
    ...
    var usingHP;
    if (savedData) {
        usingHP = savedData.usingHP;
    } else {
        usingHP = Session.get('UseHangingProtocol');
    }
    

    // Check if there is an applicable protocol
    var applicableProtocol;
    if (ViewerData[contentId] && ViewerData[contentId].applicableProtocol) {
