...
function filterToQIDOURL(server, filter) {
    ...
    return parameters; //server.qidoRoot + '/studies?' + encodeQueryData(parameters);
}
...
Services.DIMSE.Studies = function(server, filter) {
    var parameters = filterToQIDOURL(server, filter);
